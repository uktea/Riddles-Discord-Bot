...
class AnswerModal(discord.ui.Modal, title="なぞなぞに回答"):
    def __init__(self, cog: RiddleCog, source_message_id: int) -> None:
        ...
        self.add_item(
            discord.ui.Label(
                text="回答",
                description="入力内容は他の参加者には表示されません。",
...
class AnswerView(discord.ui.View):
    ...
    async def answer(
        ...
        await interaction.response.send_modal(
            AnswerModal(self.cog, interaction.message.id)
        )
...
class RiddleCog(commands.Cog):
    ...
    async def handle_answer(
        ...
        *,
        ...
        if riddle.mode == "riddle":
            await interaction.edit_original_response(
                content="回答を受け付けました。結果は期限に発表します。"
            )
            return
    ...
    async def _create_riddle(
        ...
        *,
        ...
        settings = await self._db(self.database.get_guild_settings, guild.id)
        if (
    ...
    def _open_content(
        ...
        *,
        ...
        mode_description = (
            "最初の正解で発表" if riddle.mode == "briddle" else "期限まで答えを非公開"
        )
    ...
    async def deletemydata(
        ...
        await interaction.edit_original_response(
            content=(
                ...
                "Discord上であなた自身が投稿したメッセージは削除されません。"
            )
        )
        logger.info(
