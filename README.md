...
- `/riddle`: 期限まで複数人の正解を受け付け、期限になった時点で正解を発表します。回答の公開と1人あたりの回答回数を設定できます。

どちらも問題へ画像・音声・動画を1ファイル添付できます。問題・期限・サーバー設定・個人設定はSQLiteに保存されるため、Botを再起動しても処理を再開できます。
...
| `/mypref prefid prefvalue` | 全員 | 自分が作成する `/riddle` の個人既定値を変更します。 |
...
`/riddle` の `public_answers` を `true` にすると、回数上限内で受け付けた各回答をスレッドへ公開します。正解・不正解のどちらも公開しますが、期限までは正誤を表示しません。上限を超えて拒否された回答は公開されません。`false` の場合は、ほかの参加者から回答本文は見えません。

`answer_limit` は、1人の参加者がその問題へ回答できる回数です。`0` は無制限、`1`〜`100` は指定回数までを表します。`public_answers` と `answer_limit` を省略すると、問題作成者の `/mypref` の値を使用します。`/riddle` で値を指定した場合は、その問題だけを上書きし、個人設定自体は変更しません。

`media` には画像・音声・動画を1ファイルだけ指定できます。ファイル上限は25 MiBまたはDiscordサーバーのアップロード上限の小さい方です。SVGや種類を判定できないファイルは添付できません。
...
| `answer_limit` | `0` / `1`〜`100` | `0` | 1人あたりの回答回数です。`0` は無制限です。 |

個人設定はサーバーごとに保存されます。`/mypref` の応答は実行者だけに表示されます。

### 個人データの削除

`/deletemydata` は誤操作防止のため、`confirm` に `true` を指定した場合だけ実行されます。実行すると、本人が作成した未終了問題は取り消されてスレッドが終了し、ほかの問題に保存された本人の正解者記録・回答回数・個人設定も削除されます。この操作は元に戻せません。先に `/mydata` で対象を確認してください。

`public_answers:true` によってDiscordのスレッドへ公開済みの回答メッセージは、`/deletemydata` では削除されません。削除が必要な場合はサーバー管理者へ依頼してください。

## データとセキュリティ

- SQLiteデータベースには、問題の処理に必要なDiscordサーバーID・ユーザーID・問題・正解・期限・設定などが保存されます。問題と正解者記録は結果表示に成功した後で削除され、サーバー設定と個人設定が継続して保存されます。
- Botトークン、問題の正解、参加者がModalへ入力した回答本文はログへ出力しません。
- `public_answers:true` の回答本文はDiscordのスレッドへ投稿しますが、SQLiteやログには保存しません。
...
7. 出題チャンネルを制限する場合は、そのチャンネルで管理者が `/pref allowed_channel current` を実行します。サーバー設定はサーバーごとなので、旧サーバーの設定は引き継がれません。
...
git commit -m "Add riddle preferences media and help"
git push origin main
```

`git add` には、実際に変更してコミットしたいファイルまたはディレクトリだけを指定します。共有先で更新を取り込む場合は、Botを停止してから `git pull --ff-only origin main` を実行し、依存関係を更新してBotを再起動します。
